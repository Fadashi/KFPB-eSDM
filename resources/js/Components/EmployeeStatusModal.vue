  ...
  <div v-show="show" class="fixed inset-0 overflow-y-auto bg-black bg-opacity-50 flex items-start justify-center z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-3xl my-8">
      <div class="flex justify-between items-center p-6 border-b">
        <h2 class="text-xl font-semibold">Status Pegawai</h2>
        ...
        <div class="p-6">
            ...
            <div class="inline-block min-w-full align-middle">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="status in paginatedStatuses" :key="status.id" class="hover:bg-gray-50">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
            <div class="text-sm text-gray-500 w-full sm:w-auto text-center sm:text-left">
              Menampilkan {{ startIndex + 1 }} sampai {{ endIndex }} dari {{ filteredStatuses.length }} data
            </div>
...
const statuses = ref([
  ...
  {
    id: 3,
    status_pegawai: 'Pegawai Magang'
  },
  {
    id: 4,
    status_pegawai: 'Pegawai Percobaan'
  }
]);
...
const filteredStatuses = computed(() => {
  return statuses.value.filter(status => {
    ...
    return (
      ...
      status.id.toString().includes(searchLower)
    );
  });
});

const totalPages = computed(() => {
  return Math.ceil(filteredStatuses.value.length / itemsPerPage);
});
...
const endIndex = computed(() => {
  return Math.min(startIndex.value + itemsPerPage, filteredStatuses.value.length);
});

const paginatedStatuses = computed(() => {
  return filteredStatuses.value.slice(startIndex.value, endIndex.value);
});
...
const handleSubmit = () => {
  if (isEditing.value) {
    ...
    const index = statuses.value.findIndex(s => s.id === form.id);
    if (index !== -1) {
...
const deleteStatus = (id) => {
  if (confirm('Apakah Anda yakin ingin menghapus status pegawai ini?')) {
    ...
    if (paginatedStatuses.value.length === 0 && currentPage.value > 1) {
      currentPage.value--;
    }
  }
};
